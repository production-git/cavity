...
- **0.4 Fixed:** Snap threshold reduced (`THRESH` 0.15 → 0.05) so snap is less aggressive. Guide lines made visible: skip zero-length guides, solid amber `#FFB300` lineWidth=2. Guide residue cleared on drag end (`activeSnapGuides = []` in mouseup). (`state.js::trySnap`, `renderer.js` guide case, `ui.js` mouseup)
...
- **0.5 Fixed:** Decluttered tooltips. Bond/angle/coordinate info moved to a persistent `#status-bar` at the bottom of the canvas. Tooltip is now a minimal identifier only (atom name+id, bond endpoints, cavity id). Status bar shows: atom xyz coordinates + role description on atom hover; bond length in Å on bond hover; cavity radius + center on cavity hover; polyhedron type + atom count when ≥3 atoms selected in poly mode. Canvas-drawn bond length labels and angle arcs now only render in View mode — hidden in all edit modes (Move, Poly, Add, Bonds) to keep the workspace clean. `#geom-info` floating div removed. (`index.html`, `styles.css`, `ui.js::onMouseMove`, `ui.js::updateSelUI`, `renderer.js::draw`)
